...
/**
    `obj` can be either an object or array of objects to create. If an array
        nothing gets passed back to callback.

    If an object, the following callback signature applies:
        callback(err, `obj`)
...
base.prototype.create = function(obj, callback){
    ...
    var objs = obj;
    if( !isArray ){ objs = [obj]; }
    _.each(objs, function(obj){ self._baseModelSetDefaults(obj); });
    r.table(this.name)
     .insert(objs)
     .run(this.connection, function(err, result){
        if( err ){ return callback(err); }
        if( isArray ){ return callback(); }
...
base.prototype._baseModelSetDefaults = function(obj){
